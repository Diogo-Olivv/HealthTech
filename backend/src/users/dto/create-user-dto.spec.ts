...
const pacienteMetadata: ArgumentMetadata = { type: 'body', metatype: CreatePacienteDto, data: '' };
const medicoMetadata: ArgumentMetadata = { type: 'body', metatype: CreateMedicoDto, data: '' };
...
describe('Rotas públicas de cadastro rejeitam tipo: ADMIN', () => {
  ...
  it('payload válido de paciente (sem tipo) continua sendo aceito', async () => {
    await expect(pipe.transform(pacientePayload, pacienteMetadata)).resolves.toBeDefined();
  });

  it('payload válido de médico (sem tipo) continua sendo aceito', async () => {
    await expect(pipe.transform(medicoPayload, medicoMetadata)).resolves.toBeDefined();
  });
});
